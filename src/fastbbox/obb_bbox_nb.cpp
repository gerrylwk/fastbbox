...
using DoubleArray2DIn5 = nb::ndarray<const double, nb::shape<-1, 5>, nb::c_contig, nb::device::cpu>;
...
inline OBB make_obb(double cx, double cy, double width, double height, double angle) {
...
void obb_to_corners(const OBB& box, Point* corners) {
    double cos_a = std::cos(box.angle);
    double sin_a = std::sin(box.angle);
    double hw = box.width * 0.5;
    double hh = box.height * 0.5;
    
    // Local corner coordinates (counter-clockwise from bottom-left)
    double local_x[4] = {-hw, hw, hw, -hw};
    double local_y[4] = {-hh, -hh, hh, hh};
 ...
 */
double polygon_area(const Point* vertices, int n) {
    if (n < 3) {
        return 0.0;
    }
 ...
 */
double obb_intersection_area_robust(const OBB& box1, const OBB& box2) {
    // Special case: both boxes are axis-aligned
    if (std::fabs(box1.angle) < 1e-6 && std::fabs(box2.angle) < 1e-6) {
        // Exact calculation for axis-aligned boxes
        double x1_min = box1.cx - box1.width * 0.5;
        double x1_max = box1.cx + box1.width * 0.5;
        double y1_min = box1.cy - box1.height * 0.5;
        double y1_max = box1.cy + box1.height * 0.5;
        
        double x2_min = box2.cx - box2.width * 0.5;
        double x2_max = box2.cx + box2.width * 0.5;
        double y2_min = box2.cy - box2.height * 0.5;
        double y2_max = box2.cy + box2.height * 0.5;
        
        double inter_x_min = std::max(x1_min, x2_min);
        double inter_x_max = std::min(x1_max, x2_max);
        double inter_y_min = std::max(y1_min, y2_min);
        double inter_y_max = std::min(y1_max, y2_max);
    ...
    double min_x1 = corners1[0].x, max_x1 = corners1[0].x;
    double min_y1 = corners1[0].y, max_y1 = corners1[0].y;
    for (int i = 1; i < 4; ++i) {
    ...
    double min_x2 = corners2[0].x, max_x2 = corners2[0].x;
    double min_y2 = corners2[0].y, max_y2 = corners2[0].y;
    for (int i = 1; i < 4; ++i) {
    ...
    double inter_x_min = std::max(min_x1, min_x2);
    double inter_x_max = std::min(max_x1, max_x2);
    double inter_y_min = std::max(min_y1, min_y2);
    double inter_y_max = std::min(max_y1, max_y2);
    ...
    double aabb_intersection = (inter_x_max - inter_x_min) * (inter_y_max - inter_y_min);
    
    // Apply a scaling factor based on rotation angles
    double angle_factor = std::cos(std::fabs(box1.angle)) * std::cos(std::fabs(box2.angle));
    angle_factor = std::max(0.5, angle_factor);  // Minimum 50% of AABB intersection
 ...
 */
inline double obb_area(const OBB& box) {
 ...
 * Parameters
 * ----------
 * boxes: (N, 5) float64 array [cx, cy, width, height, angle]
 * query_boxes: (K, 5) float64 array [cx, cy, width, height, angle]
 *     where angle is in radians
 *
 * Returns
 * -------
 * overlaps: (N, K) float64 array of IoU values
 */
nb::ndarray<nb::numpy, double, nb::shape<-1, -1>> bbox_overlaps_obb(
    DoubleArray2DIn5 boxes,
    DoubleArray2DIn5 query_boxes
) {
    ...
    for (size_t k = 0; k < K; ++k) {
        ...
        for (size_t n = 0; n < N; ++n) {
            ...
            double area1 = obb_area(box1);
            
            // Calculate intersection area
            double intersection_area = obb_intersection_area_robust(box1, box2);
